...
export type CanvasAction<TData = Record<string, unknown>> =
  ...
  | {
      type: 'ALIGN_LAYERS'
      payload: { targetLayerIds: string[]; align: 'left' | 'center' | 'right' | 'top' | 'middle' | 'bottom' }
    }
  | { type: 'DISTRIBUTE_LAYERS'; payload: { targetLayerIds: string[]; axis: 'horizontal' | 'vertical' } }
  | { type: 'GROUP_LAYERS'; payload: { targetLayerIds: string[]; groupId: string } }
  | { type: 'UNGROUP_LAYERS'; payload: { targetLayerIds: string[] } }
  | { type: 'ARRANGE_LAYERS'; payload: { targetLayerIds: string[]; mode: 'front' | 'back' | 'forward' | 'backward' } }
...
export function canvasReducer<TData>(
  ...
  switch (action.type) {
    ...
    case 'ALIGN_LAYERS':
      return { ...state, layers: alignLayers(state.layers, action.payload.targetLayerIds, action.payload.align) }
    ...
    case 'ARRANGE_LAYERS':
      return { ...state, layers: arrangeLayers(state.layers, action.payload.targetLayerIds, action.payload.mode) }
...
function alignLayers<TData>(
  ...
  switch (align) {
    ...
    case 'right': {
      ...
      return layers.map((l) =>
        idSet.has(l.id) ? { ...l, x: maxRight - l.width } : l,
      )
    }
    case 'center': {
      ...
      return layers.map((l) =>
        idSet.has(l.id) ? { ...l, x: center - l.width / 2 } : l,
      )
    }
    case 'top':
    ...
    case 'bottom': {
      ...
      return layers.map((l) =>
        idSet.has(l.id) ? { ...l, y: maxBottom - l.height } : l,
      )
    }
    case 'middle': {
      ...
      return layers.map((l) =>
        idSet.has(l.id) ? { ...l, y: middle - l.height / 2 } : l,
      )
    }
  }

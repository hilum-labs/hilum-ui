...
describe("canvasReducer — layer CRUD", () => {
  ...
  it("UPDATE_LAYER patches a single layer", () => {
    const state = {
      ...createInitialState(),
      layers: [makeLayer("a", 0, 0), makeLayer("b", 50, 50)],
    };
  ...
  it("UPDATE_LAYERS patches many layers in one pass", () => {
    const state = {
      ...createInitialState(),
      layers: [makeLayer("a", 0, 0), makeLayer("b", 0, 0), makeLayer("c", 0, 0)],
    };
    const next = canvasReducer(state, {
      type: "UPDATE_LAYERS",
      payload: [
...
describe("canvasReducer — viewport", () => {
  ...
  it("SET_ARTBOARD_OPACITY clamps 0–1", () => {
    const state = createInitialState();
    expect(
      canvasReducer(state, { type: "SET_ARTBOARD_OPACITY", payload: 1.5 }).artboardOpacity,
    ).toBe(1);
    expect(
      canvasReducer(state, { type: "SET_ARTBOARD_OPACITY", payload: -0.5 }).artboardOpacity,
    ).toBe(0);
  });
...
describe("canvasReducer — clipboard", () => {
  it("COPY_LAYERS captures deep clones; PASTE_LAYERS adds with offset and new ids", () => {
    ...
    const copied = canvasReducer(state, {
      type: "COPY_LAYERS",
      payload: { targetLayerIds: ["a"] },
    });

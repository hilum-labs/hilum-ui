...
const IMG_PLACEHOLDER = (
  <div className="flex size-16 items-center justify-center rounded-lg border border-taupe-200 bg-taupe-50 text-taupe-300">
    <ImageIcon size={24} />
  </div>
);
...
<MediaObject media={<img className="size-16 rounded-lg" src="..." alt="" />}>
  <h4 className="heading text-taupe-900">Lorem ipsum</h4>
  <p className="mt-1 body text-taupe-500">
...
<MediaObject
  ...
  mediaPosition="right"
>
  <h4 className="heading text-taupe-900">Media on right</h4>
  <p className="mt-1 body text-taupe-500">Content on the left, image on the right.</p>
</MediaObject>`,
...
<MediaObject
  ...
  gap="sm"
>
  <p className="body font-semibold text-taupe-900">Sofia P.</p>
  <p className="caption text-taupe-400">sofia@example.com</p>
</MediaObject>`,
...
<div className="flex flex-col gap-4">
  {comments.map((c) => (
    <MediaObject key={c.id} media={<Avatar size="sm">...</Avatar>} align="top" gap="sm">
      <p className="body font-medium text-taupe-900">{c.author}</p>
      <p className="caption text-taupe-400">{c.time}</p>
      <p className="mt-1 body text-taupe-600">{c.text}</p>
    </MediaObject>
  ))}
</div>`,
};
...
const COMMENTS = [
  ...
  { id: 2, author: "Tom Cook", time: "45 minutes ago", text: "Agreed. The typography hierarchy is a big improvement. Ready to merge.", initials: "TC", color: "bg-brand-secondary text-taupe-900" },
  { id: 3, author: "Martha Gardner", time: "10 minutes ago", text: "Shipped to staging! Let me know if there are any issues.", initials: "MG", color: "bg-taupe-900 text-white" },
];
    ...
    <div className="mx-auto max-w-7xl px-8 py-10">
      <div className="mb-10">
        <div className="caption mb-4 flex items-center gap-1.5 text-taupe-400">
          ...
          <span>/</span>
          <span className="body font-semibold text-taupe-900">Media Object</span>
        </div>
        <h1 className="display mb-2 text-taupe-900">Media Object</h1>
        ...
        <div className="mt-5 flex items-center gap-4 border-t border-taupe-100 pt-5">
          ...
          <p className="caption text-taupe-400">Avatar · Icon</p>
        </div>
      </div>
          ...
          <PreviewBlock title="Media on right" description="mediaPosition='right'" code={CODE.right} previewClassName="items-start">
            <div className="w-full max-w-lg">
              <MediaObject media={IMG_PLACEHOLDER} mediaPosition="right">
                <h4 className="heading text-taupe-900">Media on the right</h4>
                <p className="mt-1 body text-taupe-500">
            ...
            <div className="w-full max-w-sm flex flex-col gap-3">
              {COMMENTS.map((c) => (
                <MediaObject
                  ...
                  gap="sm"
                >
                  <p className="body font-semibold text-taupe-900">{c.author}</p>
                  <p className="caption text-taupe-400">{c.time}</p>
                </MediaObject>
              ))}
            </div>
              ...
              {COMMENTS.map((c) => (
                ...
                >
                  <div className="flex items-baseline gap-2">
                    <p className="body font-semibold text-taupe-900">{c.author}</p>
                    <span className="caption text-taupe-400">{c.time}</span>
                  </div>
                  <p className="mt-1 body text-taupe-600">{c.text}</p>
                </MediaObject>
              ))}

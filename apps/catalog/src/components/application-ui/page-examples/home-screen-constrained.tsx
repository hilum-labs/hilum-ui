          ...
          <div className="mx-auto w-full max-w-6xl">
            <div className="mb-6">
              <div className="caption mb-3 flex items-center gap-1.5 text-ground-400">
                ...
                <span>/</span>
                <span className="text-ground-900">Home</span>
              </div>
              <h3 className="heading text-ground-900">Program overview</h3>
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              <div className="rounded-2xl border border-ground-100 bg-white p-5">
                <div className="mb-4 flex items-center justify-between">
                  <p className="subheading text-ground-900">Projects</p>
                  <Badge variant="secondary">3 active</Badge>
                </div>
                <div className="space-y-3">
                  {PROJECTS.map((project) => (
                    <div key={project.name} className="rounded-xl bg-ground-50 p-4">
                      <p className="body font-medium text-ground-900">{project.name}</p>
                      <div className="mt-2 flex items-center justify-between">
                        <p className="caption text-ground-400">{project.status}</p>
                        <p className="caption text-ground-400">{project.due}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="rounded-2xl border border-ground-100 bg-white p-5">
                <div className="mb-4 flex items-center justify-between">
                  <p className="subheading text-ground-900">Team</p>
                ...
                <div className="space-y-4">
                  {ACTIVITY_FEED.map((item) => (
                    <div key={item.title}>
                      ...
                      <p className="caption mt-2 text-ground-400">{item.time}</p>
                    </div>
                  ))}
                </div>
              </div>

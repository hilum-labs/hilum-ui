    ...
    <div className="relative min-h-[19rem] bg-white">
      <div className="flex items-center justify-between border-b border-taupe-100 px-6 py-4">
        <div className="flex items-center gap-3">
          <div className="flex size-9 items-center justify-center rounded-xl bg-brand-secondary text-sm font-semibold text-taupe-900">
            A
          </div>
          <span className="subheading text-taupe-900">Atlas</span>
        </div>
        <div className="relative">
          ...
          {open ? (
            <div className="absolute right-0 z-10 mt-3 w-[22rem] rounded-xl border border-taupe-100 bg-white shadow-elevated ring-1 ring-taupe-100/50">
              <div className="p-2">
                {MAIN_LIST_ITEMS.map((item) => (
                  <FlyoutLink key={item.name} item={item} />
                ))}
              </div>
              <div className="rounded-b-xl bg-taupe-50 p-3">
                <p className="caption px-2 text-taupe-400">Quick links</p>
                <div className="mt-2 grid grid-cols-2 gap-2">
                  {SECONDARY_LINKS.map((link) => (
                    <a
                      key={link}
                      href={`#${link.toLowerCase().replace(/\s+/g, "-")}`}
                      className="rounded-lg px-2 py-2 body text-sm text-taupe-600 transition-colors hover:bg-white hover:text-taupe-900"
                    >
                      {link}
                    </a>
                  ))}

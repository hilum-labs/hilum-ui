...
export default function PricingComparisonTable() {
  ...
  return (
    <section className="w-full bg-taupe-50 px-8 py-16 md:px-12">
      <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] border border-taupe-100 bg-white shadow-natural">
        <div className="grid border-b border-taupe-100 md:grid-cols-[1.2fr_0.6fr_0.6fr_0.6fr]">
          <div className="px-6 py-6">
            <Badge variant="secondary" className="mb-4">Comparison table</Badge>
            <h3 className="heading text-taupe-900">A large table for teams that compare every detail</h3>
            <p className="body mt-3 max-w-xl text-taupe-500">
          ...
          <div className="border-t border-taupe-100 px-6 py-6 md:border-l md:border-t-0">
            <p className="subheading text-taupe-900">Starter</p>
            <p className="body mt-2 text-taupe-500">$39/mo</p>
          </div>
          <div className="border-t border-taupe-100 px-6 py-6 md:border-l md:border-t-0">
            <div className="flex items-center gap-2">
              <p className="subheading text-taupe-900">Growth</p>
              <Badge variant="brand">Popular</Badge>
            </div>
            <p className="body mt-2 text-taupe-500">$119/mo</p>
          </div>
          <div className="border-t border-taupe-100 px-6 py-6 md:border-l md:border-t-0">
            <p className="subheading text-taupe-900">Enterprise</p>
            <p className="body mt-2 text-taupe-500">Custom</p>
          </div>
        </div>

        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-taupe-100 bg-taupe-50">
              ...
              <th className="px-6 py-4 text-center subheading text-taupe-900">Enterprise</th>
            </tr>
          </thead>
          <tbody>
            {categories.map((category) => (
              <Fragment key={category.name}>
                <tr key={`${category.name}-header`} className="border-b border-taupe-100 bg-white">
                  <td colSpan={4} className="px-6 py-4 label text-taupe-400">
                    {category.name}
                  </td>
                </tr>
                {category.rows.map((row) => (
                  <tr key={row.feature} className="border-b border-taupe-100 last:border-b-0">
                    <td className="px-6 py-4 body text-taupe-700">{row.feature}</td>

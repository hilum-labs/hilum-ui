...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "sticky top-0 hidden h-screen flex-col border-r border-ground-100 bg-ground-50 transition-[width] duration-200 lg:flex",
...
export function MobileNavigation() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-ground-100 bg-white/95 backdrop-blur lg:hidden">
      <div className="flex h-12 items-center gap-2.5 px-4">
        <div className="flex size-6 shrink-0 items-center justify-center rounded-md bg-ground-900">
          <span className="text-[11px] font-bold leading-none text-white">D</span>
        </div>
        <span className="body font-semibold text-ground-900">Design System</span>
      </div>
      <nav
        aria-label="Primary navigation"
        className="flex gap-1 overflow-x-auto border-t border-ground-100 px-3 py-2"
      >
        {navItems.map((item) => {
          const isActive = item.href === "/" ? pathname === "/" : pathname.startsWith(item.href);

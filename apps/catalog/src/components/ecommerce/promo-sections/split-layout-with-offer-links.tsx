...
export default function SplitLayoutWithOfferLinks() {
  ...
  return (
    <div className="grid min-h-[340px] w-full overflow-hidden rounded-xl border border-taupe-100 bg-white md:grid-cols-[1.05fr_0.95fr]">
      ...
      <div className="flex flex-col justify-center p-6 md:p-8">
        ...
        <h2 className="display mt-5 text-taupe-900">Shop by offer</h2>
              ...
              <button
                ...
                className={`flex w-full items-center justify-between rounded-[22px] border px-5 py-4 text-left transition ${
                  active
                    ...
                    : "border-taupe-100 hover:border-taupe-200 hover:bg-taupe-50"
                }`}
              >
                <div>
                  <p className="body font-semibold text-taupe-900">{offer.name}</p>
                  <p className="caption mt-1 max-w-xs text-taupe-400">{offer.description}</p>
                </div>
                <span className={`flex items-center gap-1 body ${active ? "text-brand-primary" : "text-taupe-500"}`}>
                  Shop
                  <ArrowRight size={16} />
                </span>
              </button>

    ...
    <section className="w-full bg-white px-6 py-8 md:px-8">
      <div className="overflow-hidden rounded-[2rem] border border-taupe-100 bg-white">
        {PRODUCTS.map((product) => (
          <article
            key={product.name}
            className="grid gap-4 border-b border-taupe-100 px-5 py-4 last:border-b-0 lg:grid-cols-[112px_1fr_auto]"
          >
            <img src={product.image} alt={product.name} className="aspect-square w-28 rounded-2xl object-cover" />

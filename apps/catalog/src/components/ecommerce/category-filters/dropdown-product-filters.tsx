    ...
    <section className="min-h-[320px] w-full bg-white px-6 py-8 sm:px-8">
      <div className="rounded-[28px] border border-taupe-100 bg-white p-5">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <label className="flex items-center gap-3 rounded-full border border-taupe-200 px-4 py-2">
            <span className="body text-taupe-600">Sort</span>
            <select
              value={topBarSort}
              onChange={(event) => setTopBarSort(event.target.value)}
              className="body bg-transparent text-taupe-900 outline-none"
            >
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {["Flight Pack", "Weekender Pouch", "Canvas Layer"].map((name, index) => (
            ...
            >
              ...
              />
              <p className="subheading mt-4 text-taupe-900">{name}</p>

...
export default function InlineActionsSidebar() {
  ...
  return (
    <section className="min-h-[320px] w-full bg-white lg:grid lg:grid-cols-[260px_1fr]">
      <aside className="border-b border-taupe-100 p-6 lg:border-b-0 lg:border-r">
        <div>
          <p className="subheading text-taupe-900">Sort</p>
        ...
        <div className="mt-6">
          <p className="subheading text-taupe-900">Subcategories</p>
          <div className="mt-3 space-y-2">
              ...
              (item) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-2xl px-3 py-2 body transition-colors ${
                    subcategory === item
                      ? "bg-brand-primary/10 text-taupe-900"
                      : "bg-taupe-50 text-taupe-600 hover:bg-taupe-100"
                  }`}
                >
                  {item}
                  <span className="caption text-taupe-400">12</span>
                </button>
              )
            )}
          </div>
        ...
        <div className="mt-6 space-y-4">
          {(Object.keys(expandableGroups) as SidebarExpandableKey[]).map((group) => (
            <div key={group}>
              ...
              >
                <span className="subheading text-taupe-900">{group}</span>
                <ChevronDown
      ...
      <div className="p-6">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-taupe-100 pb-4">
          <div>
            <p className="subheading text-taupe-900">{subcategory}</p>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {["Compression Case", "Cord Roll", "Passport Sleeve"].map((item, index) => (
            <div
              key={item}
              className="rounded-[24px] border border-taupe-100 bg-taupe-50 p-4"
            >
              <div
              ...
              />
              <p className="subheading mt-4 text-taupe-900">{item}</p>

...
export default function InvoiceListQuickActions() {
  ...
  return (
    <div className="w-full bg-ground-50 p-6">
      <div className="space-y-4 rounded-[32px] border border-ground-200 bg-white p-6 shadow-natural">
        {ORDERS.map((order) => (
          <div key={order.id} className="relative rounded-[24px] border border-ground-100 p-5">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <div className="flex flex-wrap items-center gap-3">
                  <p className="subheading text-ground-900">{order.id}</p>
                  <StatusBadge status={order.status} />
                  <p className="caption text-ground-500">{order.date}</p>
                </div>
              ...
              <div className="flex items-center gap-3 self-start">
                <div className="text-right">
                  ...
                  <p className="body font-medium text-ground-900">{formatCurrency(order.total)}</p>
                </div>
                <button
                  type="button"

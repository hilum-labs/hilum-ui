...
export default function SimpleIconGrid() {
  return (
    <section className="w-full rounded-xl border border-taupe-100 bg-white px-6 py-16">
      <div className="grid gap-6 md:grid-cols-3">
        {coreIncentives.map((item) => (
          <div key={item.title} className="space-y-4">
            <item.icon size={26} className="text-brand-primary" strokeWidth={1.9} />
            <p className="subheading text-taupe-900">{item.title}</p>
            <p className="caption leading-relaxed text-taupe-400">{item.description}</p>
          </div>
        ))}
      </div>
    </section>

    ...
    <div className="rounded-[24px] border border-ground-200 bg-ground-50 p-5">
      <div className="space-y-3">
        <div className="flex items-center justify-between body text-ground-500">
          <span>Subtotal</span>
          <span className="text-ground-900">{formatCurrency(ORDER.subtotal)}</span>
        </div>
        <div className="flex items-center justify-between body text-ground-500">
          <span>Shipping</span>
          <span className="text-ground-900">{formatCurrency(ORDER.shipping)}</span>
        </div>
        <div className="flex items-center justify-between body text-ground-500">
          <span>Tax</span>
          <span className="text-ground-900">{formatCurrency(ORDER.tax)}</span>
        </div>
      </div>
      <div className="mt-4 flex items-center justify-between border-t border-ground-200 pt-4">
        <p className="subheading text-ground-900">Total</p>
        <p className="heading text-ground-900">{formatCurrency(ORDER.total)}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="w-full bg-white p-6">
      <div className="space-y-8 rounded-[32px] border border-ground-200 bg-white p-8 shadow-natural">
        <div className="flex flex-col gap-4 rounded-[28px] bg-ground-50 p-6 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-4">
            <div className="flex size-14 items-center justify-center rounded-full bg-brand-secondary text-ground-900">
              <CheckCircle size={24} />
            </div>
            <div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div className="rounded-[24px] border border-ground-100 p-5">
            <p className="label text-ground-400">Shipping address</p>
            <p className="subheading mt-3 text-ground-900">Ava Mitchell</p>
            <p className="body mt-2 text-ground-500">123 Market Street</p>
            <p className="body text-ground-500">San Francisco, CA 94105</p>
            <p className="body text-ground-500">United States</p>
          </div>
          <div className="rounded-[24px] border border-ground-100 p-5">
            <p className="label text-ground-400">Billing info</p>
            <p className="subheading mt-3 text-ground-900">Visa ending in 4242</p>
            <p className="body mt-2 text-ground-500">Receipt sent to {receiptEmail}</p>
            <p className="body text-ground-500">Update channel: {updateChannel}</p>
          </div>
        </div>
        <TotalsCard />
      </div>

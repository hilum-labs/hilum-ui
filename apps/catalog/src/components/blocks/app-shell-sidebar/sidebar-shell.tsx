...
export default function SidebarShell() {
  ...
  return (
    <div className="relative flex h-[520px] overflow-hidden rounded-xl border border-ground-100 bg-ground-50 shadow-natural">
      ...
      >
        {/* Logo */}
        <div className="flex h-14 items-center gap-2 border-b border-ground-100 px-4">
            ...
            <DropdownMenuTrigger asChild>
              <button className="flex min-h-10 w-full items-center gap-2.5 rounded-lg px-2.5 caption text-ground-700 transition-colors hover:bg-ground-50">
                ...
                <div className="flex-1 min-w-0 text-left">
                  <p className="caption font-semibold text-ground-900 truncate">Tom Cook</p>
                  <p className="caption-xs text-ground-400 truncate">tom@example.com</p>
                </div>
      ...
      <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
        {/* Top bar */}
        <header className="flex h-14 items-center gap-3 border-b border-ground-100 bg-white px-4">
          <Button
            variant="ghost"
            size="sm"
            className="size-8 p-0 text-ground-500 lg:hidden"
            onClick={() => setSidebarOpen(true)}
          >
            {sidebarOpen ? <X size={16} /> : <Menu size={16} />}
          </Button>
          <div className="flex-1" />
          <button className="relative text-ground-400 hover:text-ground-700">
            <Bell size={15} />

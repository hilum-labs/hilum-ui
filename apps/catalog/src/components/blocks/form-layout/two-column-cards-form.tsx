    ...
    fields: (
      <div className="flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-3">
          <Field label="First name" htmlFor="tc-fn">
            <Input id="tc-fn" placeholder="Jane" />
          </Field>
          <Field label="Last name" htmlFor="tc-ln">
            <Input id="tc-ln" placeholder="Smith" />
          </Field>
        </div>
  ...
  return (
    <div className="flex flex-col gap-5 w-full">
      {CARD_SECTIONS.map((section) => (
        <form
          key={section.id}
          className="rounded-xl border border-ground-100 bg-white shadow-natural overflow-hidden"
